...
contract PlugBalanceTest is Test {
    ...
    function test_enforce_Encoding() public {
        bytes memory terms = connector.encode(address(this), address(0), nativeType, belowOperator, belowBalance);
        (address decodedHolder, address decodedAsset, uint8 decodedType, uint8 decodedOperator, uint256 decodedBalance)
    ...
    function test_enforce_BelowNativeBalance_Exceeded() public {
        ...
        bytes memory terms = connector.encode(address(this), address(0), nativeType, belowOperator, expected);
        vm.expectRevert(abi.encodeWithSelector(PlugLib.ThresholdExceeded.selector, expected, balance));
        connector.enforce(terms, plugsHash);
    }

    function test_enforce_AboveNativeBalance() public view {
        bytes memory terms = connector.encode(address(this), address(0), nativeType, aboveOperator, aboveBalance);
        connector.enforce(terms, plugsHash);
    }

    function testRevert_enforce_AboveNativeBalance_Insufficient() public {
        uint256 expected = balance + 1;
        bytes memory terms = connector.encode(address(this), address(0), nativeType, aboveOperator, expected);
        vm.expectRevert(abi.encodeWithSelector(PlugLib.ThresholdInsufficient.selector, expected, balance));
        connector.enforce(terms, plugsHash);
    }

    function test_enforce_BelowERC20Balance() public view {
        bytes memory terms = connector.encode(address(this), address(mockERC20), erc20Type, belowOperator, belowBalance);
        connector.enforce(terms, plugsHash);
    }

    function test_enforce_BelowERC20Balance_Exceeded() public {
        uint256 expected = balance - 1;
        bytes memory terms = connector.encode(address(this), address(mockERC20), erc20Type, belowOperator, expected);
        vm.expectRevert(abi.encodeWithSelector(PlugLib.ThresholdExceeded.selector, expected, balance));
        connector.enforce(terms, plugsHash);
    }

    function test_enforce_AboveERC20Balance() public view {
        bytes memory terms = connector.encode(address(this), address(mockERC20), erc20Type, aboveOperator, aboveBalance);
        connector.enforce(terms, plugsHash);
    }

    function testRevert_enforce_AboveERC20Balance_Insufficient() public {
        uint256 expected = balance + 1;
        bytes memory terms = connector.encode(address(this), address(mockERC20), erc20Type, aboveOperator, expected);
        vm.expectRevert(abi.encodeWithSelector(PlugLib.ThresholdInsufficient.selector, expected, balance));
        connector.enforce(terms, plugsHash);
    }

    function test_enforce_BelowERC721Balance() public view {
        bytes memory terms = connector.encode(address(this), address(mockERC721), erc721Type, belowOperator, 2);
        connector.enforce(terms, plugsHash);
    }

    function test_enforce_BelowERC721Balance_Exceeded() public {
        uint256 expected = 0;
        bytes memory terms = connector.encode(address(this), address(mockERC721), erc721Type, belowOperator, expected);
    ...
    function test_enforce_AboveERC721Balance() public view {
        bytes memory terms = connector.encode(address(this), address(mockERC721), erc721Type, aboveOperator, 0);
        connector.enforce(terms, plugsHash);
    }

    function testRevert_enforce_AboveERC721Balance_Insufficient() public {
        uint256 expected = 1 + 1;
        bytes memory terms = connector.encode(address(this), address(mockERC721), erc721Type, aboveOperator, expected);
