...
import {Test, PlugLib, PlugTypesLib, PlugAddressesLib, PlugEtcherLib, PlugFactory, Plug, PlugVaultSocket, PlugMockEcho} from '../abstracts/test/Plug.Test.sol';
...
contract PlugTest is Test {
	...
	function test_PlugEmptyEcho_ExternalSolver_NotCompensated() public {
		...
		PlugTypesLib.Plug[] memory plugsArray = new PlugTypesLib.Plug[](1);
		plugsArray[0] = createPlug(PLUG_NO_VALUE, PLUG_EXECUTION);
		PlugTypesLib.LivePlugs memory livePlugs = createLivePlugs(
			plugsArray,
			0,
			0,
			solver
