...
import {Test, PlugLib, PlugEtcherLib, PlugTypesLib, Plug, PlugFactory, PlugVaultSocket, PlugMockEcho} from '../abstracts/test/Plug.Test.sol';
...
contract PlugVaultSocketTest is Test {
	...
	function testRevert_UninitializedImplementation() public {
		...
		vm.expectRevert(
			abi.encodeWithSelector(
				PlugLib.ImplementationInvalid.selector,
				uint16(2)
			)
		);
		factory.deploy(salt);
