...
contract PlugCaller {
    ...
    /**
     * @notice Initialize the contract with a slot that will hold
     *         the actively decoded address as well as a
     *         reentrency flag for the last bit.
     */
    constructor() {
        assembly {
    ...
    /**
     * @notice Receive ETH and perform a low-level operation.
     */
    receive() external payable {
        assembly {
            /// @dev Store the size of the return data.
            mstore(0x0c, sload(returndatasize()))
            /// @dev Return the address stored in the slot.
