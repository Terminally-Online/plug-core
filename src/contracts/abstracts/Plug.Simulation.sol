...
abstract contract PlugSimulation is PlugCore {
        ...
        for (uint8 i; i < $plugs.length; i++) {
            ...
            for (uint8 j; j < plug.pins.length; j++) {
                ...
                for (uint8 l; l < livePin.pin.fuses.length; l++) {
                    $results[results.length - plugsLength--] = simulate(plug, livePin, livePin.pin.fuses[l]);
                }
            }
        }
    }
    ...
    function simulate(
        ...
        view
        virtual
        returns (PlugSimulationLib.Result[] memory)
    {
        if ($indexes.length == 0) return new PlugSimulationLib.Result[](0);

        PlugSimulationLib.Result[] memory $results = new PlugSimulationLib.Result[]($indexes.length);

        for (uint256 i; i < $indexes.length;) {
            uint32 index = $indexes[i];

            PlugTypesLib.Plug memory plug = $livePlugs[uint8(index)].plugs.plugs[uint8(index >> 8)];
            PlugTypesLib.LivePin memory livePin = plug.pins[uint8(index >> 16)];

            $results[i] = simulate(plug, livePin, livePin.pin.fuses[uint8(index >> 24)]);
    ...
    function simulate(
        ...
        view
        virtual
        returns (PlugSimulationLib.Result memory $result)
    {
        bytes32 pinHash = getLivePinHash($livePin);

        (bool $success, bytes memory $callback) = address($fuse.neutral).staticcall(
            abi.encodeWithSelector(IFuse($fuse.neutral).enforceFuse.selector, $fuse.live, $plug.current, pinHash)
        );

        $result = PlugSimulationLib.Result({ success: $success, callback: $callback });
    }
     ...
     *         uint8 | 24 | fuseIndex
     */
    function indexes(PlugTypesLib.Plug[] calldata $plugs) public view virtual returns (uint24[] memory) {
        ...
        for (uint8 i; i < $plugs.length; i++) {
            ...
            for (uint8 j; j < plug.pins.length; j++) {
                ...
                for (uint8 k; k < pin.fuses.length; k++) {
                    fuseIndexes[fuseIndexes.length - $fusesLength--] = uint24((k << 16) | (j << 8) | (i << 0));
                }
            }
        }
     ...
     *         uint8 | 24 | fuseIndex
     */
    function indexes(PlugTypesLib.LivePlugs[] calldata $livePlugs) public view virtual returns (uint32[] memory) {
        ...
        returns (uint32[] memory)
    {
        ...
        for (uint256 i; i < indexesLength; i++) {
            uint24[] memory fuseIndexes = indexes(uint8($fusesLength), $livePlugs[i].plugs.plugs);
     ...
     *
     */
    function length(PlugTypesLib.Plug[] calldata $plugs) public view virtual returns (uint256 $total) {
        for (uint8 i; i < $plugs.length; i++) {
            PlugTypesLib.Plug memory plug = $plugs[i];
     ...
     *
     */
    function length(PlugTypesLib.LivePlugs[] calldata $livePlugs) public view virtual returns (uint256 $total) {
