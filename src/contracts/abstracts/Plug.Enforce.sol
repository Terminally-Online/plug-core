...
abstract contract PlugEnforce is PlugTypes {
	...
	function _enforceSignature(
		...
		if (signatureType & 0x03 == signatureType) {
            // TODO: Implement the threshold signature proving method.
	...
	function _enforceFuse(
		...
		bytes32 $pinHash
	) internal returns (bool $success, bytes memory $through) {
	 ...
	 */
	function _enforceCurrent(
		PlugTypesLib.Current memory
	) internal view virtual returns (bool $allowed) {
		$allowed = true;
	}
}
