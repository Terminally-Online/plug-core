 ...
 *      and Plugs.
 * @author @nftchance
 * @author @nftchance/plug-types (2024-02-04)
...
abstract contract PlugTypes {
    ...
    bytes32 constant PLUG_TYPEHASH = keccak256(
        "Plug(Current current,Fuse[] fuses,bytes32 salt)Current(address ground,uint256 voltage,bytes data)Fuse(address neutral,bytes live,bool forced)"
    );
    ...
    bytes32 constant PLUGS_TYPEHASH = keccak256(
        "Plugs(Plug[] plugs)Current(address ground,uint256 voltage,bytes data)Fuse(address neutral,bytes live,bool forced)Plug(Current current,Fuse[] fuses,bytes32 salt)"
    );
    ...
    bytes32 constant LIVE_PLUGS_TYPEHASH = keccak256(
        "LivePlugs(Plugs plugs,bytes signature)Current(address ground,uint256 voltage,bytes data)Fuse(address neutral,bytes live,bool forced)Plug(Current current,Fuse[] fuses,bytes32 salt)Plugs(Plug[] plugs)"
    );
    ...
    {
        if (domainHash != 0x0) {
            revert("Types:already-initialized.");
        }
    ...
    {
        $hash = keccak256(
            abi.encode(PLUGS_TYPEHASH, getPlugArrayHash($input.plugs))
        );
    }
